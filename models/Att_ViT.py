...
default_cfgs = {
    ...
    'att_vit_tiny_patch16_224': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
            'Ti_16-i21k-300ep-lr_0.001-aug_none-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz'),
    'att_vit_small_patch16_224': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
            'S_16-i21k-300ep-lr_0.001-aug_light1-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz'),
    'att_vit_small_patch32_224': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
            'S_32-i21k-300ep-lr_0.001-aug_light1-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz'),
    'att_vit_base_patch16_224': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
            'B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz'),
    'att_vit_base_patch32_224': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # we can drop out random rows/patch
        x = self.pos_drop(x + self.pos_embed)
        x, attention_profile = self.blocks(x)
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]), attention_profile
        else:
            return x[:, 0], x[:, 1], attention_profile

    def forward(self, x):
        x, attention_profile = self.forward_features(x)
        if self.head_dist is not None:
            x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
                return x, x_dist, attention_profile
            else:
                return (x + x_dist) / 2, attention_profile
        else:
            x = self.head(x)
...
@register_model
def att_vit_tiny_patch16_224(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
...
@register_model
def att_vit_small_patch16_224(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
...
@register_model
def att_vit_small_patch32_224(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/32)
    """
...
@register_model
def att_vit_base_patch16_224(pretrained=False, **kwargs):
...
@register_model
def att_vit_base_patch32_224(pretrained=False, **kwargs):
